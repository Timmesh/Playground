...
import com.playground.entities.LineItem;
import com.playground.entities.Order;
...
public class StreamsFlatMap4 {
	public static void main(String[] args) {
		...
		/*
		 * Stream<List<LineItem>> listStream = orders.stream() .map(order ->
		 * order.getLineItems());
		 * 
		 * Stream<LineItem> lineItemStream = orders.stream() .flatMap(order ->
		 * order.getLineItems().stream());
		 */

		// sum the line items's total amount
		BigDecimal sumOfLineItems = orders.stream().flatMap(order -> order.getLineItems().stream()) // Stream<LineItem>
		...
		BigDecimal sumOfOrder = orders.stream().map(order -> order.getTotal()) // Stream<BigDecimal>
				.reduce(BigDecimal.ZERO, BigDecimal::add); // reduce to sum all
